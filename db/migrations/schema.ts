...
export const healthcareWorkers = pgTable("healthcare_workers", {
	...
	index("idx_healthcare_workers_fatigue_score").using("btree", table.fatigueScore.asc().nullsLast().op("int4_ops")),
...
export const nurseAvailability = pgTable("nurse_availability", {
	...
	index("idx_nurse_availability_effective_dates").using("btree", table.effectiveFrom.asc().nullsLast().op("date_ops"), table.effectiveUntil.asc().nullsLast().op("date_ops")),
	index("idx_nurse_availability_worker_day").using("btree", table.workerId.asc().nullsLast().op("int4_ops"), table.dayOfWeek.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const nurseSkillAssignments = pgTable("nurse_skill_assignments", {
	...
	index("idx_nurse_skill_assignments_expiry").using("btree", table.expiryDate.asc().nullsLast().op("date_ops")).where(sql`(expiry_date IS NOT NULL)`),
	index("idx_nurse_skill_assignments_worker_skill").using("btree", table.workerId.asc().nullsLast().op("int4_ops"), table.skillId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.workerId],
			foreignColumns: [healthcareWorkers.workerId],
...
export const patientAcuity = pgTable("patient_acuity", {
	...
	index("idx_patient_acuity_department_date").using("btree", table.departmentId.asc().nullsLast().op("int4_ops"), table.recordedDate.asc().nullsLast().op("int4_ops")),
	index("idx_patient_acuity_shift_type").using("btree", table.shiftType.asc().nullsLast().op("enum_ops")),
	foreignKey({
...
export const timeOffRequests = pgTable("time_off_requests", {
	...
	index("idx_time_off_requests_dates").using("btree", table.startDate.asc().nullsLast().op("date_ops"), table.endDate.asc().nullsLast().op("date_ops")),
...
export const shiftBreaks = pgTable("shift_breaks", {
	...
	index("idx_shift_breaks_assignment_id").using("btree", table.assignmentId.asc().nullsLast().op("int4_ops")),
	index("idx_shift_breaks_interrupted").using("btree", table.wasInterrupted.asc().nullsLast().op("bool_ops")).where(sql`(was_interrupted = true)`),
	index("idx_shift_breaks_type").using("btree", table.breakType.asc().nullsLast().op("enum_ops")),
	foreignKey({
...
export const shiftSwapRequests = pgTable("shift_swap_requests", {
	...
	index("idx_shift_swap_requests_requesting_worker").using("btree", table.requestingWorkerId.asc().nullsLast().op("int4_ops")),
	index("idx_shift_swap_requests_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	index("idx_shift_swap_requests_target_worker").using("btree", table.targetWorkerId.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const attendanceRecords = pgTable("attendance_records", {
	...
	index("idx_attendance_records_assignment_id").using("btree", table.assignmentId.asc().nullsLast().op("int4_ops")),
	index("idx_attendance_records_overtime").using("btree", table.overtimeMinutes.asc().nullsLast().op("int4_ops")).where(sql`(overtime_minutes > 0)`),
	index("idx_attendance_records_scheduled_times").using("btree", table.scheduledStart.asc().nullsLast().op("timestamp_ops"), table.scheduledEnd.asc().nullsLast().op("timestamp_ops")),
	foreignKey({
			columns: [table.assignmentId],
			foreignColumns: [shiftAssignments.assignmentId],
...
export const workloadMetrics = pgTable("workload_metrics", {
	...
	index("idx_workload_metrics_assignment_id").using("btree", table.assignmentId.asc().nullsLast().op("int4_ops")),
	index("idx_workload_metrics_overtime_triggered").using("btree", table.overtimeTriggered.asc().nullsLast().op("bool_ops")).where(sql`(overtime_triggered = true)`),
	foreignKey({
			columns: [table.assignmentId],
			foreignColumns: [shiftAssignments.assignmentId],
...
export const feedback = pgTable("feedback", {
	...
	index("idx_feedback_ratings").using("btree", table.rating.asc().nullsLast().op("int4_ops"), table.workloadRating.asc().nullsLast().op("int4_ops")),
	index("idx_feedback_shift_id").using("btree", table.shiftId.asc().nullsLast().op("int4_ops")),
	index("idx_feedback_worker_id").using("btree", table.workerId.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const fatigueAssessments = pgTable("fatigue_assessments", {
	...
	index("idx_fatigue_assessments_risk_level").using("btree", table.riskLevel.asc().nullsLast().op("enum_ops")),
	index("idx_fatigue_assessments_score").using("btree", table.fatigueRiskScore.asc().nullsLast().op("int4_ops")),
	index("idx_fatigue_assessments_worker_date").using("btree", table.workerId.asc().nullsLast().op("date_ops"), table.assessmentDate.asc().nullsLast().op("date_ops")),
	foreignKey({
			columns: [table.workerId],
			foreignColumns: [healthcareWorkers.workerId],
...
export const shifts = pgTable("shifts", {
	...
	index("idx_shifts_auto_generated").using("btree", table.autoGenerated.asc().nullsLast().op("bool_ops")),
	index("idx_shifts_department_time").using("btree", table.departmentId.asc().nullsLast().op("timestamp_ops"), table.startTime.asc().nullsLast().op("timestamp_ops")),
	index("idx_shifts_start_time").using("btree", table.startTime.asc().nullsLast().op("timestamp_ops")),
	index("idx_shifts_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	index("idx_shifts_type").using("btree", table.shiftType.asc().nullsLast().op("enum_ops")),
	foreignKey({
			columns: [table.departmentId],
			foreignColumns: [departments.deptId],
...
export const shiftAssignments = pgTable("shift_assignments", {
	...
	index("idx_shift_assignments_confirmed").using("btree", table.confirmedAt.asc().nullsLast().op("timestamp_ops")).where(sql`(confirmed_at IS NOT NULL)`),
	index("idx_shift_assignments_shift_id").using("btree", table.shiftId.asc().nullsLast().op("int4_ops")),
	index("idx_shift_assignments_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
...
export const complianceViolations = pgTable("compliance_violations", {
	...
	index("idx_compliance_violations_detected_at").using("btree", table.detectedAt.asc().nullsLast().op("timestamp_ops")),
	index("idx_compliance_violations_type").using("btree", table.violationType.asc().nullsLast().op("enum_ops")),
	index("idx_compliance_violations_unresolved").using("btree", table.resolvedAt.asc().nullsLast().op("timestamp_ops")).where(sql`(resolved_at IS NULL)`),
	index("idx_compliance_violations_worker_id").using("btree", table.workerId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.workerId],
			foreignColumns: [healthcareWorkers.workerId],
