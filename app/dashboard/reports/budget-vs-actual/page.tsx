...
const TabContent = ({ tabValue, periodId }: { tabValue: string; periodId: number }) => {
  ...
  const { data, isLoading, isError } = useBudgetVsActualAggregateByProject(periodId, selectedProjectCode, !!periodId)

  console.log("=== Budget vs Actual Debug Info ===");
  ...
  // Log specific goods and services data
  const goodsAndServicesRow = data?.find(row => row.description === "Goods and services");
  if (goodsAndServicesRow) {
    console.log("Goods and Services Row:", goodsAndServicesRow);
    console.log("- Budget (previous):", goodsAndServicesRow.previous);
    console.log("- Actual (current):", goodsAndServicesRow.current);
  } else {
    console.log("Goods and Services row not found in data");
  }
  ...
  if (isError) {
    return <div className="bg-white p-6 rounded-lg border">Failed to load budget vs actual report for {tabValue.toUpperCase()}</div>
  }

  // Transform data for the component
  const transformed = data ?? []

  return <BudgetVsActualStatement initialData={transformed as any} />
}
